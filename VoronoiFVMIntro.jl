...
# ╔═╡ 345d8924-e743-4ecd-a396-b98d464e3661
begin
...
using Triangulate: Triangulate, TriangulateIO, triangulate, plot_in_out
...
begin
	...
	using GridVisualize: scalarplot, scalarplot!, gridplot, reveal
    GridVisualize.default_plotter!(CairoMakie)
end

# ╔═╡ ed0485da-8840-4990-b20b-8102127f8b1b
using SimplexGridFactory: SimplexGridBuilder, point!, facet!, facetregion!, options!

# ╔═╡ 79d579af-db38-49f6-beae-08246b41b09a
begin
    using VoronoiFVM: VoronoiFVM, solve, ramp, unknowns, fbernoulli_pm
...
PlutoUI.ExperimentalLayout.hbox([
    ...
    ```
    Approximate flux between neigboring control volumes ``\omega_k,\omega_l`` by finite difference expression ``\mathbf g(\mathbf u_k , \mathbf u_l)`` involving ``\mathbf u_k=\mathbf u(\vec x_k)``, ``\mathbf u_l=\mathbf u(\vec x_l)``. 	
    """,
])


# ╔═╡ 4edb6c05-e731-42ba-8b95-b800aa705a77
md"""
...
md"""
Let ``N`` be the number of control volumes ``\omega_k`` / collocation points ``\vec x_k``. For ``k=1\dots N``, 
``i=2 \dots M``, ``t=t_{i}``, ``t^\text{old}=t_{i-1}``, assuming ``u_k=u(\vec x_k, t_i), u_k^\text{old}=u(\vec x_k, t_{i-1})``,
the implicit Euler two point flux scheme writes

```math
|\omega_k|\frac{\mathbf s(\mathbf u_k)-\mathbf s(\mathbf u^{\text{old}}_k)}{t-t^\text{old}} + \displaystyle\sum_{l\in\mathcal N_k} \frac{|\sigma_{kl}|}{h_{kl}}\mathbf g(\mathbf u_k , \mathbf u_l) + |\omega_k|\mathbf r(\mathbf u_k) + |\gamma_k|\mathbf a(\mathbf u_k) = |\omega_k| \mathbf f_k + |\gamma_k|\mathbf b_k
```
...
With exception of ``\mathbf{\vec j}``, all constitutive functions introduced above can be used in the discrete version as well. The flux ``\mathbf{\vec j}`` is replaced by the discrete edge flux ``\mathbf g: \mathbb{R}^n \times \mathbb{R}^n \to \mathbb{R}^n.``
...
md"""
The Voronoi cells are created from Delaunay triangulations. As a demonstration, we show how to use `Triangulate.jl`, a Julia wrapper of J. R. Shewchuk's `Triangle` code to create a Voronoi tesselation.  It is created from a boundary conforming Delaunay grid (warranted by the "D"-flag in the control string).
"""

# ╔═╡ 259a117d-9346-4386-af31-2742baba6a7f
begin
    triin = TriangulateIO()
    # Point describing the boundary
    triin.pointlist =
    ...
    # Region points, region markers, region volumes
...
plot_in_out(
    ...
    triin,
    triout;
    figure = CairoMakie.Figure(; size = (700, 400)),
    voronoi = vorout,
...
# ╔═╡ b7cdb7cf-50c2-4b07-a59c-9d95173bfd96
md"""
These grids and functions on them can be plotted using the `GridVisualize.jl` package.
"""

# ╔═╡ ad4d2af1-8b7e-486d-bfe4-75687d2f9651
gridplot(grid, size = (300, 300))

# ╔═╡ f7e96c35-4ca6-4767-bdac-b9179339dffa
md"""
The visualization reflects the basic information held by an `ExtendableGrid`.
It consists of:
"""

# ╔═╡ 4cabef08-24e2-469f-90f9-7f345490c42b
md"""
- Point coordinates:
"""

# ╔═╡ 0069dd56-099c-4d5c-90be-2311e42bf1ca
grid[Coordinates]

# ╔═╡ 0cce7529-3b94-4384-a059-7aecbde5cced
md"""
- Simplex connectivity:
"""

# ╔═╡ 4254df40-5b7c-49b4-a65e-46fdd0017e21
grid[CellNodes]

# ╔═╡ 3b36a7af-1318-4fb5-8db5-f232550f6a68
md"""
- Cell region markers (color coded in pastel colors):
"""

# ╔═╡ f5061fd1-2f55-4cb5-8b5f-b0c80de919b0
grid[CellRegions]

# ╔═╡ 89f501af-9dd1-491b-928a-35c31eff2211
md"""
- Exterior and interior boundary faces: 
"""

# ╔═╡ 6c030464-f698-4afb-b507-7cbf86f4517b
grid[BFaceNodes]

# ╔═╡ dcb0b1c2-a02b-475a-aac5-33afad9117bd
md"""
- Boundary region markers (color coded in bright colors):
"""

# ╔═╡ 93551b77-3af9-4d18-a5b8-67ea8d6e2a8c
grid[BFaceRegions]

# ╔═╡ 2ea40e6f-01c6-404a-9cfd-bdd4cd2810fe
md"""
More information can be created on demand, but is mostly not necessary for VoronoiFVM.jl
"""

# ╔═╡ 8f5e6f2b-a33f-429e-96f3-6470f0f7155d
md"""
### Grid generation examples
ExtendableGrids provides a number of simpler grid generation methods.
"""

# ╔═╡ 11a17759-ab02-456a-81f5-85aad0e6fc88
X = 0:0.5:10

# ╔═╡ 94f98b39-0275-49e2-ac37-4a42136e19b1
grid1d = simplexgrid(X)

# ╔═╡ 0cb17977-af38-432c-9aa1-a458abd34f5b
gridplot(grid1d, size = (700, 150), legend = :lt)

# ╔═╡ a98e75d4-404b-4769-8350-fea721637aa8
grid2d = simplexgrid(X, X)

# ╔═╡ c49b73de-5608-49f6-98b5-3b80193f135b
gridplot(grid2d, size = (400, 400))

# ╔═╡ c6473460-d91d-4874-9cf4-e04d6bccfcab
grid3d = simplexgrid(X, X, X)

# ╔═╡ 90aa6087-88b3-4b6b-b92d-71516cfc62cc
import PlutoVista

# ╔═╡ e780ca28-15e7-4328-afc0-7a557989fb1e
gridplot(grid3d, Plotter = PlutoVista, xplanes = [5])

# ╔═╡ 7ef31b64-445e-471e-ace5-7e61a5ec0fcc
begin
    ...
    grid2dsf = simplexgrid(builder)

end

# ╔═╡ 4b8483f2-aade-4ca1-a140-5bd95f7186d3
gridplot(grid2dsf, size = (400, 400))

# ╔═╡ 0ee8a722-b2a5-41e1-b662-58edad013522
md"""
## Examples I
"""
...
# ╔═╡ 677c3989-3510-4d51-a6a0-679c9bd15299
md"""
- Specify grid:
"""

# ╔═╡ be6491d7-de06-4a5c-846d-74908721c889
rd1dgrid = simplexgrid(0:0.01:1)

# ╔═╡ 5f5ef720-22d7-47a6-8a02-81391794a6fa
md"""
...
# ╔═╡ 5399b0dc-524f-4d4b-b6a3-b9240228abf7
md"""
- Specify parameters, create and solve system:
"""

# ╔═╡ 8b3c97f1-9d4b-4110-951d-a851c419d4fa
rd1ddata = (D = 2, R = 100)

# ╔═╡ 39d70b33-87e3-4866-a528-fb6e51054c9f
rdphysics=VoronoiFVM.Physics(flux=rdflux, reaction=rdreaction, breaction=rdbc)

# ╔═╡ 45be9b74-c047-4389-89d5-609719a33e83
rd1dsystem = VoronoiFVM.System(
    rd1dgrid,
	rdphysics;
    species = [1]
)
...
rd1dsol = solve(rd1dsystem; data=rd1ddata)
...
gridplot(rd2dgrid, linewidth=0.1,size=(700,300))

# ╔═╡ 74073959-df42-46d7-896e-68bc78c40cc8
rd2dsystem = VoronoiFVM.System(
    rd2dgrid, rdphysics;
    species = [1]
)

# ╔═╡ 377b8314-2287-4592-a957-99fe96ca09ae
rd2dsol=solve(rd2dsystem; data=rd1ddata)
...
- Calculation of the data derived from the geometry: ``|\omega_k|``, ``|\frac{\sigma_{kl}}{h_{kl}}|``, ``|\gamma_k|``. This then allows to piece togeteher a nonlniear operator ``A: \mathbf R^N \to \mathbf R^N`` essentially defined on the graph made of the edges and nodes of the triangulation, where ``A(u)=0`` gives the solution of the discrete problem.
- Iterative solution by (damped) Newton`s method: given ``u_k``, iterate until ``||d_k||<`` tol:
```math
	\begin{aligned}
    \text{residual:}\;     r_k&= A(u_k)\\
    \text{solve}\; A'(u_k)d_k&= r_k\\ 
    \text{update:}\;  u_{k+1} &= u_k-d_k
    \end{aligned}
```
"""
...
- The values are assembled into the residual vector ``r_k``, and the local Jacobi matrices are assembled into the global Jacobi matrix ``A'(u_k)``.   Efficient sparse matrix assembly is supported by the Julia package ExtendableSparse.jl.
...
rds1dsys=VoronoiFVM.System(grid1d, rdsphysics; species=[1,2])
...
let
	u=rds1dtsol(rds1dt)
	vis=GridVisualizer(size=(700,300), limits=(-0.1,4),legend=:rt )
	scalarplot!(vis, grid1d, u[1,:], color=:red, label="u₁")
	scalarplot!(vis, grid1d, u[2,:], color=:green, label="u₂", clear=false)
	reveal(vis)
end

# ╔═╡ efc9894d-edf7-400d-8ae5-bd4111eeb413
md"""
Instead of the inbuilt implicit Euler method, it is also possible
to uses transient solvers from the Julia SciML DifferentialEquations suite
"""
...
# ╔═╡ 6449cdd9-de0d-4b16-88af-07cef64b898e
...
# ╔═╡ eda3c7c0-2184-4e7b-9360-d16d8e273660
uddflux=sedanflux!
...
let
     vis = GridVisualizer(size=(700,300), legend=:rt)
               sol = uddtsol(uddt)
            scalarplot!(vis,
                        uddgrid,
                        sol[udddata.iphi, :];
...
GridVisualize = "~1.8.2"
OrdinaryDiffEqRosenbrock = "~1.3.1"
PlutoUI = "~0.7.60"
PlutoVista = "~1.0.1"
...
manifest_format = "2.0"
project_hash = "76d9d842f67528869a2f2278b2d98b9ab944b4df"
...
git-tree-sha1 = "571614d6badd36d9093c3b0e028a9ceec70cc7b3"
...
git-tree-sha1 = "4b35936a986caf6f1fea4350eaee5f98e80deaea"
