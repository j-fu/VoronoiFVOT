...
# ╔═╡ 4882bfcd-930d-4558-aec3-15be276ccff3
using Plots

# ╔═╡ 3194fcf1-979a-41d2-a30d-dcf4ccefe4ce
using GSL_jll

# ╔═╡ 4beb48e6-3313-4340-aa21-46cbf043f9e9
using GSL

# ╔═╡ 9dd2ecf4-be55-4368-a068-1de0527fb0aa
using ForwardDiff

# ╔═╡ bcdb4152-ae60-4d76-aebe-9a9b6aa936a2
using DiffResults,LinearAlgebra

# ╔═╡ 7fc3ba32-7142-4905-baa9-20fceace54b7
...
# ╔═╡ 3f306291-d502-4612-a43f-60a7bf8bbd9b
EL.hbox([md"""
- Use of modern knowledge in language design
- Syntax comparable to matlab, python/numpy
- Just-ahead-of time compilation  to native code ``\Rightarrow`` high performance without need for vectorization
- Built-in performant multi-dimensional arrays, comprehensive linear algebra
- Efficient integration with C/C++, python, R $\dots$
...
# ╔═╡ 15a35a9a-9460-4eb9-8408-8ab91a1d9c71
md"""
## How to install and  run Julia
__Installation:__
  - [Download](https://julialang.org/downloads/) from julialang.org
    - Linux, Windows, MacOS
    - Version 1.10 with long term support
    - Current version 1.11
"""

# ╔═╡ d778de04-2b10-463b-8eb5-94952ba11b52
htl"""
<a href="https://modernjuliaworkflows.org/">Modern Julia Workflows</a><br>
<iframe src="https://modernjuliaworkflows.org/" style="width: 600px; height: 200px;"></iframe>"""

# ╔═╡ dbf21ad2-1a9b-418e-93e5-08da11649b54
htl""" 
<a href="https://techytok.com/from-zero-to-julia/">From Zero to Julia!</a><br>
<iframe src="https://techytok.com/from-zero-to-julia/" style="width: 600px; height: 200px;"></iframe>"""

# ╔═╡ 8f362593-67d5-4aac-9c6b-2bf149ebe260
md"""
...
# ╔═╡ b049c745-226f-41b0-b181-e331a1f336d3
md"""
## Pluto notebooks
"""

# ╔═╡ 4837c032-2c13-4378-952c-bab863ab83ee
md"""
Pluto notebooks are __reactive__: dependent cells are automatically recalculated:
"""

# ╔═╡ 914554c5-deb8-4f40-9af6-a04cf577f990
md"""
## Package management
"""
...
EL.grid([md"""
- __Packages__ extend Julia's core functionality. Each package is a git repository with standardized structure.
- __Package Registries__  provide the infrastructure for finding package repositories via package names
- The default __General Registry__ has ≈ 10000 Julia packages and  ≈ 1500 binary packages
- __Package Environments__ 
   - record version compatibility (in `Project.toml`) 
   - record exact package versions (in `Manifest.toml`)
   - they are transferable and thus allow for 100% __reproducible code__
...
md"""
- Julia packages have a standardized stucture which includes the interface to the package itself,
  unit testing and documentation 
- A package's `Project.toml` describes dependencies and version compatibility
- Some interesting packages:
   - Flux.jl, Lux.jl, MLJ.jl, Knet.jl, AlphaZero.jl: Machine learning
   ...
   - CUDA.jl, CUArrays.jl: Julia on GPU 
...
- The __Foreign Function Interface (FFI)__ allows to call binary code compliant with the C ABI
...
- `BinaryBuilder.jl` provides a container like infrastructure to build and maintain __binary packages__ for __all architectures supported by Julia__
   ...
   - Binary packages are handeled by the package manager as well
...
# ╔═╡ 3afd4109-ca67-4af3-9fa4-573bb978d5dc
md"""
Usually, wrapper packages with Julia like API are created around binary packages:
"""

# ╔═╡ 931efa6b-b3e9-4231-9b6a-267e7fc06645
GSL.sf_bessel_J0(6.0)

# ╔═╡ 3612cec7-b8e7-4ad8-b6cb-2c9b6a478602
md"""
## Julia type system
...
# ╔═╡ 0b8e7a1e-72e5-481f-960d-bef7243a5994
a=1.0

# ╔═╡ a51fc5c9-2b0a-4297-9f88-77bca442b623
typeof(a)

# ╔═╡ 2bfe3f23-b76b-4cc5-a725-c49eddaaa71b
md"""
- Abstract types label concepts which work for a several concrete types without regard to their memory layout etc
"""

# ╔═╡ f9e286c6-7587-4404-9874-ffc378a79735
md"""
## Functions,  Methods and Multiple Dispatch
Julia Functions can have different variants of their implementation depending
...
# ╔═╡ 715ba4f1-a947-417f-ad96-39afe9e346b5
md"""
Define a function `test_dispatch`:
"""

# ╔═╡ 0319ee9b-3670-4c4c-9aa8-eabb7b3dfae6
begin
   ...
   test_dispatch(x::AbstractArray)="abstract array: $(typeof(x)), size=$(size(x))"	
end

# ╔═╡ 15318c93-4f9d-4baf-9a32-f35ea2259c94
test_dispatch("xxx")

# ╔═╡ 9d5c72f1-a7bf-4403-b621-b93027853616
methods(test_dispatch)

# ╔═╡ fe34dcf0-d625-4718-8989-0123471154da
md"""
Julia generates specialized code for function methods depending on the type of function parameters  __if the types of the parametes is fixed__
- Paradigm for __code structuring and code extension__ 
...
- __Performant code__ for functions tailored to the particular parameter types
"""

# ╔═╡ 4fa631af-bdcd-43b0-8f18-cea68bad4f36
md"""
The `@code_native` macro shows the generated assembler code
"""

# ╔═╡ 10b10c43-da6d-44ba-83f3-7b7f91ab81a1
md"""
...
__Complex numbers__ $\mathbb C$ extend the real numbers $\mathbb R$ based on the introduction of  $i$ with $i^2=-1$.

__Dual numbers__ $\mathbb D$ extend the real numbers $\mathbb R$ by  introducing a number 
$\varepsilon$ with $\varepsilon^2=0$:
...
__Polynomials of dual numbers:__ Let $p(x)=\sum_{i=0}^n p_i x^i$. Then 
...
- ``\Rightarrow`` __forward mode automatic differentiation__
- Multivariate dual numbers: generalization for __partial derivatives__
"""

# ╔═╡ 23b9ca0f-da46-4756-a5c8-92ed7e0f4170
md"""
## A rudimentary dual number example
"""

# ╔═╡ 440a1aca-7171-42ad-95fe-ef7a0dda8be7
md"""
Define a dual number type as a parametric type
"""

# ╔═╡ e5019742-bd42-45e6-bd03-33a77260185f
begin
	struct MyDual{T} <: Number where {T <: Real}
     	v::T
	    d::T
	end
...
# ╔═╡ c081a0ea-c30d-40bd-baad-feb94f16eef6
md"""
Define some arithmetic operartions for this type
"""

# ╔═╡ 3c133925-c035-4351-a5cf-d0f7accff119
begin
	Base.:+(x::MyDual, y::MyDual) = MyDual(x.v+y.v, x.d+y.d)
    Base.:-(y::MyDual)            = MyDual(-y.v, -y.d)
    Base.:-(x::MyDual, y::MyDual) = x + -y
	Base.:*(x::MyDual, y::MyDual) = MyDual(x.v*y.v, x.v*y.d + x.d*y.v)
end

# ╔═╡ 5426f3a3-ce36-4880-9555-8909a5a4bf3d
f(x)=sin(4x);

# ╔═╡ b6a3aa8f-4376-4745-9a1d-639daf863ee3
X=-10:0.1:10; plot(X,f.(X),size=(500,80),label="")

# ╔═╡ ef6e81b2-cb70-4687-96b8-2840f8cb780a
square(x)=x*x;

# ╔═╡ 8655380e-c20b-48da-a898-7dca6f1e710f
MethodAnalysis.methodinstances(square)

# ╔═╡ 75040b26-dedb-4a3c-b064-d693e2aaff04
e=MyDual(3,1)

# ╔═╡ 7034ae7e-c376-4f7c-9d92-94b6f2d40436
typeof(e)

# ╔═╡ 8437802a-6a36-49cd-9710-405957cbbcb5
@code_native square(e)

# ╔═╡ 5fb35670-90dd-43c4-9a18-f7edfa1e12ca
md"""
Check with a polynomial
"""

# ╔═╡ 4207517c-c3ed-4273-8bed-05c1689b138a
p(x)=x^3+x^2;

# ╔═╡ 91430e7f-10ee-4b7c-a589-c70c14883144
dp(x)=3x^2+2x;

# ╔═╡ 72aa80ac-3064-4665-ab57-3cd829c6f73f
@code_native square(x)

# ╔═╡ d9a30357-7989-4899-a018-c718dbc18388
square(x)

# ╔═╡ d5f17784-fe80-4a9e-9cec-b06e413394af
p(MyDual(x))

# ╔═╡ f78fc20b-295b-480d-983f-1d13eec7292f
md"""
Note that we did not define `^` for dual numbers - this comes through the abstract number interface.
"""

# ╔═╡ be3cd1c5-321b-458c-adc9-a87f53f55214
p(x),dp(x)

# ╔═╡ 5c75ba22-e532-441f-bcfd-e544fde57a19
md"""
## Dual numbers in Julia: ForwardDiff.jl
"""

# ╔═╡ 3951dec4-9729-40fc-94b9-5dce3ea2f80c
p(ForwardDiff.Dual(x,1))

# ╔═╡ c1a7843b-1854-416a-83e0-bac2bc090375
g(x)=sin(exp(0.2*x)+cos(5x));

# ╔═╡ 43850408-0025-4a71-87d0-c2bf7a335d99
dg(x)=ForwardDiff.derivative(g,x);

# ╔═╡ 01628692-cb43-4813-8c4a-ac01b41383fd
let
	X=0:0.01:10
	p=plot(X,g.(X),size=(500,75),label="g(x)")
	plot!(p,X,dg.(X),label="dg(x)")
end

# ╔═╡ a482b601-7677-4943-867c-50a411be9145
md"""
## A simple Newton solver
"""

# ╔═╡ b8b59b81-8b48-4770-8b3d-e444a813761c
function newton(A,b;u0=zeros(length(b)), tol=1.0e-12, maxit=120)
    result=DiffResults.JacobianResult(u0) # result buffer for AD
    u=copy(u0)
    it=1
    while it<maxit
...
# ╔═╡ b4401a15-296c-485f-a350-7d33ceb7665e
md"""
Define a function `A` and right hand side `b`
"""

# ╔═╡ aa755683-efa5-47a2-8abd-81c38378f359
A(u)=[u[1]^3+2*u[2]^5+u[1]+1, 4u[2]+1];

# ╔═╡ bc91e42b-1f8b-46cb-89de-de6408cb7f67
b=[0,0];

# ╔═╡ 59d74a58-56f3-4d1b-91aa-1985c4132ece
result=newton(A,b)

# ╔═╡ c45e58bc-ebf2-4be8-af08-3061ddf12ae3
A(result)-b

# ╔═╡ 0e60498b-2ad6-4fd1-b963-893ffa363a96
md"""
...
# ╔═╡ a2542774-8d10-40b9-b076-60aa79c4c0d6
md"""
## DifferentialEquations.jl I 
"""

# ╔═╡ 0301b143-3eff-4e1c-875a-c405bae05847
...
# ╔═╡ 97931205-4d22-4d44-ac89-0ed8ed6fd45b
begin
	u0 = [1.0;0.0;.0];	
	tspan = (0.0,50.0)
	@time sol = DifferentialEquations.solve(ODEProblem(lorenz!,u0,tspan))
	plot(sol,vars=(3,1,2),size=(300,300))
end

# ╔═╡ f6b4c938-cf68-46e4-8b53-639cd2f60e2e
EL.hbox([md"""
- Multi-language suite for high-performance solvers of differential equations and scientific machine learning (SciML) components
- Solvers for  systems of ordinary differential equations (ODEs), Differential-algebraic equations (DAEs), Stochastic DEs (SDEs), Delay DEs (DDEs), ``\dots``
- Main author: Chris Rackauckas
"""])

# ╔═╡ 58c6c81d-ccda-42c3-8dd6-e6a911eb031d
md"""
Just the list of DAE solver methods from [https://diffeq.sciml.ai/stable/solvers/dae_solve/](https://diffeq.sciml.ai/stable/solvers/dae_solve/)$\dots$
"""

# ╔═╡ 34ac6ce8-ac40-462d-83e8-7137b419a826
html"""<iframe src="https://diffeq.sciml.ai/stable/solvers/dae_solve/" style="width: 650px; height: 350px;"></iframe>"""
...
# ╔═╡ b58fb910-738a-4ec1-b856-0741c17da717
EL.hbox([md"""
...
- __This presentation__ is a pluto notebook.
""",
	
hsep(),

	html"""<iframe id="installation" width="300px%" height=180px
src="https://www.youtube.com/embed/OOjKEgbt8AI" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen=""></iframe>"""
...
# ╔═╡ a81efe0e-4932-431b-bbaa-0c57a6f722ce
   html"""
    <style>
...
# ╔═╡ 1660795d-06d9-4c27-8211-9751bf56feb0
# ╠═╡ disabled = true
#=╠═╡
x=10.5
  ╠═╡ =#

# ╔═╡ cd37ec3b-aef7-41e0-ae2e-37e2aaa64644
x=Float64(3.0)
...
manifest_format = "2.0"
project_hash = "2bef06ff10021ba9fcaf6b35c91d5ecf20fe2611"
...
git-tree-sha1 = "3b612393d3cf394f508314c9e2678291971bf552"
...
git-tree-sha1 = "cdea3469429a3f20a046557d070e21797278d22b"
...
git-tree-sha1 = "ce1d3c56d84315a0f3b93c6f5d2dfe8d6015c589"
